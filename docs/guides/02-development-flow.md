...
```text
0. マイルストーン作成（/create-milestone）※ 初回またはリリース計画時
       ↓
1. タスク起票 / 課題の特定
       ↓
...
**コマンド**: `/create-milestone`

- リリース単位の計画を作成する
- 対象タスクを一覧化し、優先度と scope（lite/full）を設定する
- プロジェクト開始時または新しいリリースサイクルの開始時に実行する
- 作成後は docs/state.md の current_milestone が更新される

### Step 1: タスク起票 / 課題の特定

- PdM・エンジニア・QAでディスカッションし、何を作るか合意する
- タスク管理ツールにタスクを作成する（未導入の場合は日付で代替）
...
- マイルストーンの完全性・正確性・一貫性を3次元で検証する
- 次にすべきアクションを自動提示する
- 定期的に実行し、マイルストーンの進捗を可視化する
- 全完了条件を満たすと、マイルストーンの振り返りに進む
